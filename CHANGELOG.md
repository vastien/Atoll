...
### Fixed
- The minimalistic player's spacing. Its layout and the function that sizes its container were describing the same rhythm separately, and had drifted: the height allowed 4pt for a progress row that draws 14, and 2pt of gap above the controls that actually carried 6. The container is a fixed frame aligned to the top, so the missing ~14pt came out of the bottom and the whole stack read as squashed against it. Both now read from one set of metrics. The title and artist were also only a point apart at 12 and 10, and the title's measuring font was a point off the one it draws in, which biases the marquee; they are 13 semibold and 11 with matching metrics.
